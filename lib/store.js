...
/**
 * Lista conversaciones de un flujo (solo metadata, sin las imágenes pesadas).
 * El sessionId tiene el formato `${userId}::${slug}`, así que filtramos por el sufijo.
 * Para ver los mensajes completos (con imágenes), usar getConversation(sessionId).
 */
export async function listConversations(slug, limit = 200) {
  await ensure();
  // Solo traemos el último mensaje como JSON para el preview — el resto se omite.
  // jsonb_array_length nos da el conteo sin transferir el array entero.
  const rows = await sql()`
    SELECT
      session_id,
      updated_at,
      (data->>'cursor')::int AS cursor,
      data->>'phase'         AS phase,
      jsonb_array_length(COALESCE(data->'messages', '[]'::jsonb)) AS msg_count,
      (data->'messages'->-1) AS last_msg
    FROM progress
  ...
  `;
  return rows.map((r) => {
    const last = r.last_msg || null;
    return {
      sessionId: r.session_id,
      updatedAt: r.updated_at,
      cursor: r.cursor || 0,
      phase: r.phase || "scripted",
...
export async function getFlowStats(slug) {
  await ensure();
  const rows = await sql()`
    SELECT (data->>'cursor')::int AS cursor, data->>'phase' AS phase
    FROM progress
    WHERE session_id LIKE ${"%::" + slug}
  `;
  const cursors = rows.map((r) => r.cursor || 0);
  const phases = rows.reduce((acc, r) => {
    const p = r.phase || "scripted";
    acc[p] = (acc[p] || 0) + 1;
    return acc;
  }, {});
